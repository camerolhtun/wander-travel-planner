...
/** Curated "big" currencies float to the top of prefix results. */
const MAJOR = [
  "USD", "EUR", "GBP", "JPY", "AUD", "CAD", "CHF", "CNY", "INR", "SGD",
  "HKD", "NZD", "SEK", "NOK", "DKK", "KRW", "THB", "MYR", "IDR", "PHP",
  "VND", "AED", "ZAR", "BRL", "MXN",
];
...
const RAW: Currency[] = [
  ...
  { code: "RWF", name: "Rwandan Franc", symbol: "RFw" },
...
const majorRank = new Map(MAJOR.map((c, i) => [c, i]));

/** Majors first (in MAJOR order), then everything else alphabetically by code. */
export const CURRENCIES: Currency[] = [...RAW].sort((a, b) => {
  const ra = majorRank.get(a.code);
  const rb = majorRank.get(b.code);
...
/**
 * Code-prefix matches first (e.g. "r" → RON, RSD, RUB…), then name matches.
 * Case-insensitive; results keep the majors-first ordering above.
 */
export function searchCurrencies(query: string, limit = 8): Currency[] {
  const q = query.trim().toLowerCase();
  if (!q) return [];

  const codeStarts: Currency[] = [];
  const nameStarts: Currency[] = [];
  const contains: Currency[] = [];

  for (const c of CURRENCIES) {
    const code = c.code.toLowerCase();
    const name = c.name.toLowerCase();
    if (code.startsWith(q)) codeStarts.push(c);
    else if (name.split(/[\s-]+/).some((w) => w.startsWith(q))) nameStarts.push(c);
    else if (name.includes(q)) contains.push(c);
  }

  return [...codeStarts, ...nameStarts, ...contains].slice(0, limit);
}
